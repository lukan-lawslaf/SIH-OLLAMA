...
# 2b/4b are the primary demo models, 235b-cloud is an Ollama-cloud test model.
ALLOWED_MODELS = {
    model.strip()
    for model in os.getenv(
        "QWEN_ALLOWED_MODELS", "qwen3-vl:2b,qwen3-vl:4b,qwen3-vl:235b-cloud"
    ).split(",")
    if model.strip()
}
...
def resolve_model(requested: Any) -> str:
    """Trust the client's model when it is allow-listed, else use the default."""
    if isinstance(requested, str) and requested.strip() in ALLOWED_MODELS:
